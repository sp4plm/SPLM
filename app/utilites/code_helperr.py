...
class CodeHelperr:
    ...
    _debug_name = 'CodeHelperr'

    @staticmethod
    def write_to_file(file_path, istr='', mod='a'):
        if CodeHelperr.check_file(file_path):
            CodeHelperr.__write_to_file(file_path, mod, istr)
            return True
        return False
    ...
    def remove_file(file_path):
        if CodeHelperr.check_file(file_path):
            os.unlink(file_path)
            return not CodeHelperr.check_file(file_path)
        return False

    @staticmethod
    def add_file(file_path):
        CodeHelperr.__write_to_file(file_path)
        return not CodeHelperr.check_file(file_path)
    ...
    def read_file(file_path, mod='r'):
        txt = ''
        if CodeHelperr.check_file(file_path):
            txt = CodeHelperr.__read_from_file(file_path, mod)
        return txt
    ...
    def check_dir(dir_path):
        return CodeHelperr.check_fs_item(dir_path) and os.path.isdir(dir_path)

    @staticmethod
    def is_empty_dir(dir_path):
        if not CodeHelperr.check_dir(dir_path):
            raise Exception('Directory "{}" does not exists!'.format(dir_path))
        flist = CodeHelperr.get_dir_content(dir_path)
        return (0 == len(flist))

    @staticmethod
    def get_dir_content(dir_path):
        content = []
        if CodeHelperr.check_dir(dir_path):
    ...
    def check_file(file_path):
        return CodeHelperr.check_fs_item(file_path) and os.path.isfile(file_path)
