...
class FilesManagment(PublishModConf):
    ...
    def __init__(self):
        ...
        self._mod_config = app_api.get_config_util()(self.get_mod_path('_cfg'))
        self._root_dir = ''
        self._root_dir = os.path.join(PublishModConf.DATA_PATH, self._mod_config.get('config.Main.root_name'))
        self._cfg = None
        self._current_dir = ''
    ...
    def _init_data_struct(self):
        if not os.path.exists(PublishModConf.DATA_PATH) or not os.path.isdir(PublishModConf.DATA_PATH):
            os.mkdir(PublishModConf.DATA_PATH)
        if not os.path.exists(self._root_dir) or not os.path.isdir(self._root_dir):
            os.mkdir(self._root_dir)
        dir_list = self._mod_config.get('config.Main.dirs_list').split(',')
        for i_dir in dir_list:
            test = os.path.join(self._root_dir, i_dir)
            dir_key = 'config.Main.' + i_dir
            if not os.path.exists(test) or not os.path.isdir(test):
                os.mkdir(test)
                reg_key = dir_key + '.use_register'
                if 0 == self._mod_config.get(reg_key):
    ...
    def sync_description(self):
        ...
        pth = self._current_dir
        if files:
            if '' != descr:
                for fi in descr:
                    fi['fullname'] = os.path.join(self._current_dir, fi['name'])
    ...
    def create_item_description(self, item):
        ...
        info['fullname'] = '' # item # полный путь
        if os.path.isdir(item):
            ...
            if -1 < relative.find('res'):
                roles = ''
            roles = self.__get_section_roles(relative)
            info['name'] = relative
            info['roles'] = roles
            info['label'] = self.__get_dir_label2(relative)
        else:
    ...
    def __get_section_roles(self, sect):
        dir_list = self._mod_config.get('config.Main.dirs_list').split(',')
        roles = ''
        if sect in dir_list:
            dir_key = 'config.' + sect + '.access'
            roles = self._mod_config.get(dir_key)
        return roles
    ...
    def _init_roles(self):
        self._admin_role = self._mod_config.get('config.Main.adminRole')
        self._oper_role = self._mod_config.get('config.Main.operRole')
    ...
    def __get_dir_label2(self, dir_name):
        dir_list = self._mod_config.get('config.Main.dirs_list').split(',')
        lbl = dir_name
        if dir_name in dir_list:
            dir_key = 'config.' + dir_name + '.label'
            lbl = self._mod_config.get(dir_key)
        return lbl
